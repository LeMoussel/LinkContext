...
function getLinksList(obj) {
    ...
    if (window.responseHeaders[getKeyCase(window.responseHeaders,"X-Robots-Tag")] === 'nofollow') { 
        linkNoFollow = true
    }
    if(document.querySelector("meta[name=robots]")) {
        if (document.querySelector("meta[name=robots]").getAttribute("content").includes('nofollow')) {
            linkNoFollow = true
        }
    }
    ...
    for (var i = 0; i < col.length; i++) {
        ...
        a = "<link " + 
            "ID='#ID#' " + 
            "Name=\"" + code(pt) + "\" " +
            "Adr='" + (new URL(code(link.getAttribute('href')), location)).href + "' " + 
            "External='" + (link.host !== location.host) + "' " +
            "NoFollow='" + linkNoFollow + "' " +
            "Weight='" + (1 - window.numLinks / window.totalLinks).toFixed(5) + "'" +
            "/>"
        var c = CryptoJS.MD5(a);
        list.push(c);
        t += a.replace('#ID#', c) + "\n";
...
function getViXMLObject(obj, level, pid) {
    ...
    if (obj.terminal()) {
        ...
        internal += getImageList(obj);
        //internal += spc + getText(obj);
        internal += getText(obj);
        ...
        block += "<Block Ref='Block" + pid + "' leafblock='1' internal_id='" + obj.label + "' ID='" + CryptoJS.MD5(internal) + "' Pos='WindowWidth||PageRectLeft:" + obj.dim.x + " WindowHeight||PageRectTop:" + obj.dim.y + " ObjectRectWidth:" + obj.dim.w + " ObjectRectHeight:" + obj.dim.h + "' Doc='' importance='" + obj.importance + "'>\n"
        block += internal;
    } else {
